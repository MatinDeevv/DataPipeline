"""Feature artifact references and loading helpers."""

from __future__ import annotations

import datetime as dt

import polars as pl
from pydantic import BaseModel, Field

from mt5pipe.storage.parquet_store import ParquetStore
from mt5pipe.storage.paths import StoragePaths


class FeatureArtifactRef(BaseModel):
    """Stable reference to a persisted feature-view artifact family."""

    feature_key: str = Field(..., min_length=1)
    clock: str = Field(..., min_length=1)
    artifact_id: str | None = None


def load_feature_artifact(
    paths: StoragePaths,
    store: ParquetStore,
    ref: FeatureArtifactRef,
    *,
    date_from: dt.date | None = None,
    date_to: dt.date | None = None,
) -> pl.DataFrame:
    """Load a persisted feature view by registry key and clock."""
    if date_from is None or date_to is None:
        return store.read_dir(paths.root / "feature_views" / f"feature={ref.feature_key}" / f"clock={ref.clock}")

    frames: list[pl.DataFrame] = []
    current = date_from
    while current <= date_to:
        day = store.read_dir(paths.feature_view_dir(ref.feature_key, ref.clock, current))
        if not day.is_empty():
            frames.append(day)
        current += dt.timedelta(days=1)

    if not frames:
        return pl.DataFrame()
    return pl.concat(frames, how="diagonal_relaxed").sort("time_utc")
